...
function AdminContractsContent() {
    ...
    const allMappedCustomers = useMemo(() => {
        ...
        // "계약등록" 상태만 필터링 (전체 고객)
        return mapped.filter(c => String(c['진행구분']) === '계약등록');
    ...
    const filteredCustomers = useMemo(() => {
        ...
        return sortedCustomers.filter((c: any) => {
            const searchMatch = !searchTerm || (
                ...
                (c['유입채널'] && String(c['유입채널']).includes(searchTerm))
            );
    ...
    return (
        <div className="lg:px-4 lg:py-6 space-y-6 pb-32">
